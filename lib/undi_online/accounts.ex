defmodule UndiOnline.Accounts do
  ...
  alias UndiOnline.Accounts.{Account, User, UserToken, UserNotifier}
  ...
  def notify_subscribers({:ok, %{users: [user|_]} = account} = result) do
    Phoenix.PubSub.broadcast(UndiOnline.PubSub, "account_created", %{account: account, email: user.email})
    result
  end
  ...
  def subscribe_on_account_created do
    Phoenix.PubSub.subscribe(UndiOnline.PubSub, "account_created")
  end
