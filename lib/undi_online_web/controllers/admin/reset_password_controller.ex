defmodule UndiOnlineWeb.Admin.ResetPasswordController do
  use UndiOnlineWeb, :controller

  alias UndiOnline.{Admins, Emails, Mailer, Admins.Guardian}

  plug UndiOnlineWeb.Plugs.RedirectAdmin
  ...
  def create(conn, %{"admin" => %{"email" => email}}) do
    if admin = Admins.get_admin_by_email(email) do
      token = Phoenix.Token.sign(UndiOnlineWeb.Endpoint, "admin_auth", admin.id)
  ...
  def show(conn, %{"token" => token}) do
    case Phoenix.Token.verify(UndiOnlineWeb.Endpoint, "admin_auth", token, max_age: 600) do
      {:ok, id} ->
