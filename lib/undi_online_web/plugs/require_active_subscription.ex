defmodule UndiOnlineWeb.Plugs.RequireActiveSubscription do
  ...
  def call(conn, _opts) do
    with "" <> _ <- Application.get_env(:stripity_stripe, :api_key),
         true <- Application.get_env(:undi_online, :require_subscription),
         %{} = account <- conn.assigns[:current_account],
         %{stripe_id: "" <> _} <- Billing.get_billing_customer_for_account(account)
    do
      UndiOnline.Billing.get_active_subscription_for_account(account.id)
  ...
  defp handle_inactive_subscription(%UndiOnline.Billing.Subscription{}, conn), do: conn

  defp handle_inactive_subscription(_, conn) do
    case Application.get_env(:undi_online, :require_subscription) do
      false ->
        conn
