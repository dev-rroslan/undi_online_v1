defmodule UndiOnlineWeb.Router do
  ...
  pipeline :session_layout do
    plug :put_root_layout, {UndiOnlineWeb.LayoutView, :session}
  end

  # Set the layout for when an admin signs in
  pipeline :admin_session_layout do
    plug :put_root_layout, {UndiOnlineWeb.Admin.LayoutView, :session}
  end

  # Set the main layout for the actual application
  pipeline :app_layout do
    plug :put_root_layout, {UndiOnlineWeb.App.LayoutView, :root}
  end

  # Set the main layout for the admin area
  pipeline :admin_layout do
    plug :put_root_layout, {UndiOnlineWeb.Admin.LayoutView, :root}
  end

  pipeline :require_active_subscription do
    plug UndiOnlineWeb.Plugs.RequireActiveSubscription
  end
  ...
  scope "/app" do
    ...
    live "/subscriptions/new", UndiOnlineWeb.SubscriptionLive.New, :new
    delete "/sign_out", UndiOnlineWeb.UserSessionController, :delete
    get "/settings/confirm_email/:token", UndiOnlineWeb.UserSettingsController, :confirm_email
  end
  ...
  scope "/app", UndiOnlineWeb.App, as: :app do
  ...
  if Application.get_env(:undi_online, :env) == :dev do
  ...
  if Application.get_env(:undi_online, :env) in [:dev, :test] do
